...
const CategorySelectorComponent = (props) => {
    ...
    const renderButtons = props.buttonNames.map((item, index) => (
        <StyledButton id={props.buttonIds[index]} className={[index === props.currentContentIndex && "selected", "top-button"]} key={index + "-CategorySelectorButton"} selected={index === props.currentContentIndex} onClick={props.onClick}>
